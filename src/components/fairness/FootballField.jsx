  ...
  const analysis = useMemo(() => {
    const methods = METHODOLOGIES
      ...
      .map((m) => {
        ...
        // Bar color logic
        let barColor = '#C9A84C'; // within range
        if (offerPrice < low) barColor = '#1E2D45'; // below range
        if (offerPrice > high) barColor = '#4ade80'; // above range
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="rounded-lg border p-4" style={{ backgroundColor: '#141B2D', borderColor: '#1E2D45' }}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="text-sm font-semibold" style={{ color: '#C9A84C' }}>Valuation Ranges (EGP per Share)</h4>
          <span className="ml-auto text-xs px-2 py-0.5 rounded" style={{ backgroundColor: '#0A0E17', color: '#8892A4' }}>
            Offer Price: <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: offerPrice > 0 ? '#C9A84C' : '#8892A4' }}>
        ...
        <div className="grid grid-cols-3 gap-4">
          {METHODOLOGIES.map((m) => (
            <div key={m.key} className="p-3 rounded border" style={{ backgroundColor: '#0A0E17', borderColor: '#1E2D45' }}>
              <p className="text-xs font-medium mb-2" style={{ color: '#F0F4FF' }}>{m.label}</p>
        ...
        <>
          <div className="rounded-lg border overflow-hidden" style={{ backgroundColor: '#141B2D', borderColor: '#1E2D45' }}>
            <div className="px-4 py-2 border-b" style={{ backgroundColor: '#0A0E17', borderColor: '#1E2D45' }}>
              <h3 className="text-sm font-semibold" style={{ color: '#C9A84C' }}>Football Field Chart</h3>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis
                    ...
                    label={{ value: 'EGP per Share', position: 'insideBottom', offset: -5, fill: '#8892A4', fontSize: 11 }}
                  />
                  <YAxis
                    dataKey="label"
                    type="category"
          ...
          <div className="rounded-lg border overflow-hidden" style={{ backgroundColor: '#141B2D', borderColor: '#1E2D45' }}>
            <div className="px-4 py-2 border-b" style={{ backgroundColor: '#0A0E17', borderColor: '#1E2D45' }}>
              <h3 className="text-sm font-semibold" style={{ color: '#C9A84C' }}>Per-Methodology Analysis</h3>
            </div>
            <div className="p-4 overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: '#0A0E17' }}>
                    <th className="text-left px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>Methodology</th>
                    <th className="text-right px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>Low</th>
                    <th className="text-right px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>Midpoint</th>
                    <th className="text-right px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>High</th>
                    <th className="text-right px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>Premium / (Discount)</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold" style={{ color: '#C9A84C' }}>Within Range</th>
                  </tr>
                </thead>
                <tbody>
                  {analysis.methods.map((m, i) => (
                    <tr key={m.key} style={{ backgroundColor: i % 2 === 0 ? '#141B2D' : '#182236' }}>
                      <td className="px-4 py-2 text-xs font-medium" style={{ color: '#F0F4FF' }}>{m.label}</td>
                      <td className="px-4 py-2 text-right text-xs" style={{ fontFamily: "'IBM Plex Mono', monospace", color: '#F0F4FF' }}>
                        {formatNumber(m.low, 2)}
                      </td>
                      <td className="px-4 py-2 text-right text-xs" style={{ fontFamily: "'IBM Plex Mono', monospace", color: '#8892A4' }}>
                      ...
                      <td className="px-4 py-2 text-right text-xs font-medium" style={{
                        ...
                        color: m.premiumToMidpoint >= 0 ? '#4ade80' : '#f87171',
                      }}>
          ...
          <div className="rounded-lg border overflow-hidden" style={{ backgroundColor: '#141B2D', borderColor: '#1E2D45' }}>
            <div className="px-4 py-2 border-b" style={{ backgroundColor: '#0A0E17', borderColor: '#1E2D45' }}>
              <h3 className="text-sm font-semibold" style={{ color: '#C9A84C' }}>Fairness Determination</h3>
            </div>
            <div className="p-6">
              {/* Verdict */}
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <span className="text-lg font-bold" style={{
                    ...
                    color: analysis.isFair ? '#4ade80' : '#f87171',
                  }}>
              ...
              <div className="px-4 py-3 rounded border" style={{
                ...
                <p className="text-sm italic" style={{ color: '#F0F4FF', lineHeight: 1.6 }}>
                  {analysis.isFair
              ...
              <div className="mt-4 flex items-start gap-2 px-3 py-2 rounded" style={{ backgroundColor: '#0A0E17' }}>
      ...
      {!hasData && (
        <div className="flex flex-col items-center justify-center py-16 rounded-lg border" style={{ backgroundColor: '#141B2D', borderColor: '#1E2D45' }}>
          <Scale className="w-8 h-8 mb-3" style={{ color: '#8892A4' }} />
